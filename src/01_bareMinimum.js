...
_.slice = function (arr, start, end) {
  ...
  if (start < 0) _start = Math.max(0, arr.length + start); //arr의 시작인덱스 //만약 [-1] = 마지막인덱스 = arr.length-1
  if (end < 0) _end = Math.max(0, arr.length + end);//arr의 끝인덱스
  ...
  for (let i = _start; i < _end; i++) { //start인덱스부터 end인덱스직전까지
    result.push(arr[i]);
  }
...
// ex) arr = [1, 2, 3, 4, 5] n=3 arr[2]
_.last = function (arr, n) {
  // TODO: 여기에 코드를 작성합니다.
  if(n===undefined || n<0) return [arr[arr.length-1]];  //[5]
  if(n > arr.length){
    let newArr = arr;
    return newArr;
  }
  let newArr2 = [];
  //마지막 3개 = 3,4,5 -> arr.length-n = 5-3=2
  //마지막 4개 = 2,3,4,5 ->5-4=1
...
_.each = function (collection, iteratee) {
  // TODO: 여기에 코드를 작성합니다.
  if(Array.isArray(collection)){ //collection이 배열이 맞다면
    for(let i=0; i<collection.length; i++){ //인덱스를 0부터 콜렉션배열길이까지 반복한다.
      iteratee(collection[i], i, collection);//i함수(콜렉션요소, 콜렉션인덱스, 콜렉션배열)
    }
  }else{
    for(let j in collection){//콜렉션에서 키를 하나씩 꺼내온다
...
// _.filter는 test 함수를 통과하는 모든 요소를 담은 새로운 배열을 리턴합니다. //test(요소)===true
...
_.filter = function (arr, test) {
  ...
  _.each(arr, function (item) { //item은 arr의 요소를 하나씩 꺼내온다는 뜻
...
_.uniq = function (arr) {
  // TODO: 여기에 코드를 작성합니다.
  let result = [];
//result안에 item이 없다면 item을 result에 push하기
  _.each(arr, function (item) { //item은 arr의 요소를 하나씩 꺼내온다는 뜻
...
//arr = [1,2,3,4,5] keyOrIdx=4 ----> return arr[keyOrIdx]=arr[4]=5
_.pluck = function (arr, keyOrIdx) {
  ...
  _.map(arr, function (item) { //item은 arr의 요소를 하나씩 꺼내온다는 뜻
    result.push(item[keyOrIdx]);
    //if(Array.isArray(item)){ //배열이 맞다면 a=[[1],[2]]
    //  result.push(item[keyOrIdx]); //item = [1] keyOrIdx=0 item[keyOrIdx]=1
    //}else{ //객체라면 a = [{b:!},{c:2}]
    //  result.push(item[keyOrIdx]);//item = {b:1} keyOrIdx=0  item[keyOrIdx]=1
    //}
  });
  return result;
};
...
_.reduce = function (arr, iteratee, initVal) {
  ...
  //return 배열의 누적값
  //each 함수 쓰기
  //초기값이 있으면 배열 모든 요소 i함수에 전달
  //초기값이 없으면 배열 첫번째 요소 =acc, 배열의 2번째 요소부터 반복작업
...
//  iteratee(acc, ele, idx, arr)
  ...
  _.each(arr, function (item, index) { //item의 인덱스가0일때, 즉,처음 배열의 요소를 돌때는
    if(initVal === undefined && index===0){ //초기값을 입력받지 않았다면
      result = item; //초기값은 첫번째요소
      //2번째요소부터 반복작업을 해야한다는 코드는?
    }else{
    result= iteratee(result, item, index, arr);
    }
  });
  return result;
};
